...
export const getUserEmail = (uid: string): Promise<string | undefined> => {
    return auth.getUser(uid).then((record: UserRecord) => record.email);
};

export const getPremiumUsers = async (): Promise<string[]> => {
    const users: string[] = await getUsers();
    const premium: { uid: string, premium: boolean }[] = await Promise.all(users.map((uid) => isUserPremium(uid).then((p: boolean) => ({
        uid,
        premium: p
    }))));
